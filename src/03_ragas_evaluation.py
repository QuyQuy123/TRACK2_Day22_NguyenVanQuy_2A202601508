...
def run_rag(retriever, llm, prompt, question: str) -> dict:
    ...
    """
    # 1. Retrieve documents từ retriever
    docs = retriever.invoke(question)

    # 2. Tạo contexts là danh sách page_content (KHÔNG ghép chuỗi ở đây)
    contexts = [doc.page_content for doc in docs]

    # 3. Ghép contexts thành 1 string để truyền vào {context} của prompt
    ctx_str = "\n\n".join(contexts)

    # 4. Chạy chain (prompt | llm | StrOutputParser()).invoke(...)
    answer = (prompt | llm | StrOutputParser()).invoke({
        "context":  ctx_str,
        "question": question,
    })

    # 5. Trả về dict với answer và contexts (list)
